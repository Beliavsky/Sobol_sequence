...
!  Discussion:
!
!    This routine uses the default integer precision.
!
!  Example:
!
!       N    Binary    Hi 1
...
!    will always be 0.
!
!    Output, integer I4_BIT_HI1, the number of bits base 2.
!
  implicit none
...
!
!    Output, integer I4_BIT_LO0, the position of the low 1 bit.
!
  implicit none
...
!
  ...
  integer dim_num
  integer, parameter :: dim_max = 1111
  integer, parameter :: log_max = 30

  integer atmost
  integer, save :: dim_num_save = -1
  integer i
  ...
  integer k
  integer l
  integer, save, dimension(dim_max) :: lastq
  integer m
  integer, save :: maxcol
  integer newv
  integer, save, dimension(1:dim_max) :: poly
  real quasi(dim_num)
  real, save :: recipd
  integer seed
  integer, save :: seed_save = - 1
  integer seed_temp
  integer, save, dimension(1:dim_max,1:log_max) :: v
...
!
  quasi(1:dim_num) = real ( lastq(1:dim_num) ) * recipd
  lastq(1:dim_num) = ieor ( lastq(1:dim_num), v(1:dim_num,l) )
...
!    An input value of 0 requests the first (0-th) element of the sequence.
!
!    Output, real ( kind = rk ) QUASI(DIM_NUM), the next quasirandom vector.
!
  implicit none
  ...
  real ( kind = rk ), dimension ( dim_num ) :: quasi
  real ( kind = rk ), save :: recipd
  integer ( kind = 8 ) ::  seed
...
!
!    Output, real ( kind = rk ) R(M,N), the points.
!
  implicit none
...
!
!    Output, integer TAU_SOBOL, the value TAU.
!
  implicit none
